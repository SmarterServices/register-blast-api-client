...
client.prototype.updateAppointmentDetails = function (campusKey, appointmentId, body) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var requestObj = {
      url: `${_this.basePath}/campus/${campusKey}/appointments/${appointmentId}`,
      method: 'POST',
      json: body,
      headers: {
    ...
    _this.requestWithAutoLogin(requestObj, resolve, reject, true);
  });
};
...
client.prototype.getAvailability = function (body) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var requestObj = {
      url: `${_this.basePath}/campus/availability`,
      method: 'POST',
      json: body,
      headers: {
        authorization: `Bearer ${_this.token}`,
...
/**
   * getCampusDetails Returns the details for a single campus
   * @param  {String} campusKey key that relates to a campus
