 ...
 */
client.prototype.requestWithAutoLogin = function requestWithAutoLogin(requestObj, resolve, reject) {
  let config = {
  ...
  if (!this.token || currentTime > this.tokenExpirationTime) {
    this.requestWithLogin(requestObj, this.getAutoLoginCallback(requestObj, config));
  } else {
    request(requestObj, this.getAutoLoginCallback(requestObj, config));
  }
}
 ...
 * @return {callback}
 */
client.prototype.getAutoLoginCallback = function getAutoLoginCallback(requestObj, config) {
  let _this = this;
